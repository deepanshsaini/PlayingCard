...
class PlayingCardView: UIView {
    @IBInspectable var rank: Int = 11 { didSet { setNeedsDisplay(); setNeedsDisplay() } }
    @IBInspectable var suit: String = "♥️" { didSet { setNeedsDisplay(); setNeedsDisplay() } }
    @IBInspectable var isFaceUp: Bool = true { didSet { setNeedsDisplay(); setNeedsDisplay() } }

    private func centeredAttributedString(_ string: String, fontSize: CGFloat) -> NSAttributedString {
        var font = UIFont.preferredFont(forTextStyle: .body).withSize(fontSize)
        //MARK:- Sets Dynamic Size for old people or who in need
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            if let faceCardImage = UIImage(named: rankString+suit, in: Bundle(for: self.classForCoder),compatibleWith: traitCollection) {
                faceCardImage.draw(in: bounds.zoom(by: SizeRatio.faceSizeCardImageSizeToBoundsSize))
            } else {
                drawPips()
            }
        }else{
...
extension PlayingCardView {
    private struct SizeRatio {
        ...
        static let faceSizeCardImageSizeToBoundsSize: CGFloat = 0.75
    }
